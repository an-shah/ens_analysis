...
DimPlot(obj.wt, reduction = "umap.cca", group.by = "type", label = TRUE)
p <- FeaturePlot(obj.wt, 'Ret', reduction = 'umap.cca', max.cutoff = 2) + xlim(-10,7) + ylim(-8, 9) + coord_equal()
print(p)
ggsave(paste0('/Volumes/backup-harddrive/figures/figure 3/wt_Ret_featplot.svg'), plot = p)
# library(EnhancedVolcano)
# obj.wt = JoinLayers(obj.wt)
# Idents(obj.wt) = 'type'
# markers = FindMarkers(obj.wt, ident.1 = c('iMN_12') , ident.2 = c('iMN_8/9'))
...
DimPlot(obj.het, reduction = "umap.cca", group.by = "cca_clusters.5", label = TRUE)
p <- FeaturePlot(obj.het, 'Ret', reduction = 'umap.cca', max.cutoff = 2) +xlim(-7, 11) + ylim(-10,8) + coord_equal()
print(p)
ggsave(paste0('/Volumes/backup-harddrive/figures/figure 3/het_Ret_featplot.svg'), plot = p)
# obj.het = JoinLayers(obj.het)
# Idents(obj.het) = 'cca_clusters.2'
# markers = FindMarkers(obj.het, ident.1 = c('11') , ident.2 = c('13'))
# p = EnhancedVolcano(markers, rownames(markers), x = 'avg_log2FC', y = 'p_val_adj') + xlim(-10,10)  + ylim(0, 80)
# print(p)
```

```{r, fig.width = 3, fig.height=5}
p <- VlnPlot(obj, "Ret", group.by = "genotype", sort = "increasing", pt.size = 0.1)
print(p)
ggsave(paste0('/Volumes/backup-harddrive/figures/figure 3/Ret_expression.svg'), plot = p)
```
...
for (g in c('Ntng1')){  
  p = FeaturePlot(obj.het, g, reduction = "umap.cca", max.cutoff = 2) + NoLegend()+ coord_equal()
...
obj.neuron = subset(obj.E12.5, type==c('iMN_8/9', 'iMN_12'))
cluster.sample <- table(obj.neuron$type, obj.neuron$genotype) %>%
  ...
  cluster.sample$sample <- factor(cluster.sample$sample, levels = c('iMN_8/9', 'iMN_12'))
p = ggplot(cluster.sample, aes(x=cluster, y=count, fill=sample)) +
    geom_bar(stat="identity") +
    theme_minimal()  
print(p)
...
obj.neuron = subset(obj.E14.5, type==c('iMN_8/9', 'iMN_12'))
cluster.sample <- table(obj.neuron$type, obj.neuron$genotype) %>%
    as.data.frame.matrix() %>%
    rownames_to_column(var = "sample")
  cluster.sample[-1] <- lapply(cluster.sample[-1], function(x) x/sum(x))
...
cluster.sample$sample,
  levels = c('eMN_7', 'eMN_1', 'iMN_12', 'iMN_8/9', 'progenitor', 'neuroblast')
)
